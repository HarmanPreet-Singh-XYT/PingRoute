        ...
        child: Tooltip(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: _isHovered ? widget.colors.accent : widget.colors.borderColor,
            ...
            child: Row(
              children: [
                Icon(widget.icon, size: 20, color: _isHovered ? widget.colors.accent : widget.colors.textSecondary),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(widget.label, style: widget.type.caption, overflow: TextOverflow.ellipsis),
                      const SizedBox(height: 2),
                      Text(
                        widget.value,
                        style: widget.type.title.copyWith(color: widget.valueColor ?? widget.colors.textPrimary, fontSize: 18),
...
List<(IconData, String, String, Color?)> statTileData(
  ...
  final maxLatency = validHops.isEmpty ? 0 : validHops.map((s) => s['max'] as int).where((v) => v != -1).fold<int>(0, (a, b) => b > a ? b : a);
  final avgLatencies = validHops.map((s) => s['avg']).whereType<num>().where((v) => v != -1).toList();
  final avgLatency = avgLatencies.isEmpty ? 0 : (avgLatencies.reduce((a, b) => a + b) / avgLatencies.length).round();
  
  final lastJitter = deepStats.isNotEmpty && (deepStats.last['jitter'] as List).isNotEmpty
  ...
  return [
    ...
    (FluentIcons.speed_high, 'Max latency', '${maxLatency}ms', latencyColor(colors, maxLatency)),
    (FluentIcons.analytics_view, 'Avg latency', '${avgLatency}ms', latencyColor(colors, avgLatency)),
    (FluentIcons.activity_feed, 'Jitter (last hop)', '${lastJitter}ms', null),
    (FluentIcons.warning, 'Packet loss (target)', '$finalPacketLoss%', finalPacketLoss > 0 ? colors.latencyBad : colors.latencyGood),
  ];
}

/// Key/value row list for a single hop's detail stats with hover copy support.
class StatTable extends StatelessWidget {
  const StatTable({super.key, required this.colors, required this.type, required this.rows});
  final AppColors colors;
  final AppTypography type;
                ...
                Row(
                  ...
                  children: [
                    ...
                    if (_isHovered) ...[
                      ...
                      Icon(FluentIcons.copy, size: 12, color: widget.colors.accent),
                    ],
                  ],
                ),
              ],
...
class GraphTypeColumn extends StatelessWidget {
  const GraphTypeColumn({super.key, required this.dataType, required this.onSelect});
